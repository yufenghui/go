...
func (s *segment) String() string {
	...
	buf.WriteString("[\n")
	buf.WriteString(fmt.Sprintf("\tbucketsLen : %d\n", s.bucketsLen))
	buf.WriteString(fmt.Sprintf("\tpairTotal : %d\n", s.pairTotal))
	buf.WriteString(fmt.Sprintf("\tpairRedistributor : %s\n", s.pairRedistributor))
	for i, b := range s.buckets {
		buf.WriteString(fmt.Sprintf("\tbuckets[%d]: %s\n", i, b))
	}
	buf.WriteString("]")
