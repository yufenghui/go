...
const (
	...
	BUCKET_STATUS_NORMAL = 0
	// BUCKET_STATUS_UNDERWEIGHT 代表散列桶过轻。
	BUCKET_STATUS_UNDERWEIGHT = 1
	// BUCKET_STATUS_OVERWEIGHT 代表散列桶过重。
	BUCKET_STATUS_OVERWEIGHT = 2
)
...
type PairRedistributor interface {
	...
	Redistribe(bucketStatus BucketStatus, buckets []Bucket) (newBucket []Bucket, changed bool)
	// String 会返回当前再分布器的字符串表示形式。
	String() string
}
...
type myPairRedistributor struct {
	...
	// emptyBucketCount 代表空的散列桶的计数。删除操作才会更新该字段值。
...
func (pr *myPairRedistributor) String() string {
	var myPairRedistributorStringTemplate = "PairRedistributor: upperThreshold: %d, overweightBucketCount: %d, emptyBucketCount: %d"
	return fmt.Sprintf(myPairRedistributorStringTemplate,
		atomic.LoadUint64(&pr.upperThreshold),
		atomic.LoadUint64(&pr.overweightBucketCount),
		atomic.LoadUint64(&pr.emptyBucketCount))
}
